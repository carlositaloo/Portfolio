...
def aguardar_imagem(imagem_path, timeout=30, intervalo=0.1, confidence=0.8, continuar=False, reverter=False):
    ...
    while time.time() - inicio < timeout:
        ...
        try:
            ...
            if reverter:
                # Modo reverso: espera a imagem DESAPARECER
                if posicao:
                    # Imagem ainda está presente, continua aguardando
                    pass
                else:
                    # Imagem desapareceu, pode avançar
                    return None
            else:
                # Modo normal: espera a imagem APARECER
                if posicao:
                    return posicao
                    
        except Exception as e:
            if reverter:
                # Se houve erro ao procurar, considera que não encontrou (desapareceu)
                return None
    ...
    if continuar:
        ...
        if reverter:
            raise TimeoutError(f"Imagem {imagem_path} ainda presente após {timeout}s")
        else:
...
pessoas = aguardar_imagem('img\\funcionario.png')
...
aguardar_imagem('img\\funcionario_menu.png')
clicar((591, 282))
sleep_cancelamento(0.5)
clicar((826, 282))
...
print(f"CPF/Senha: {numeros}")
clicar((1279, 859))  # Fechar detalhe funcionário

# time.sleep(0.5)
# aguardar_imagem('img\\semUsuario.png', timeout=10, confidence=0.9, reverter=True)
# pyautogui.click(953, 340, duration=0.15)
# pyautogui.hotkey('ctrl', 'c')
# cpf = pyperclip.paste()
# print(f"CPF/Números extraídos: {cpf}")
...
Através do link: https://academico.aiamis.com.br/FrameHTML/web/app/RH/PortalMeuRH/#/login
""")
print(f"""
A senha do usuário {usuario} {nome} foi redefinida para o CPF do mesmo (somente números). Ao acessar pela primeira vez, será solicitado que altere a senha:
