...
timeset = 0.4
incremento_cord = 19  # Defina a variável aqui
...
def cadastro_Serie(cord, digito):
    ...
    return cord + incremento_cord  # Use a variável aqui

def verificarerror():
    if aguardar_imagem('img\\error.png', timeout=4):
        pyautogui.click(1160, 552, duration=0.15) # CANCELAR
        sleep_cancelamento(timeset)
        pyautogui.click(1276, 819, duration=0.15) # CONFIRMAR CANCELAR
        sleep_cancelamento(timeset)
        print(" *")
    else:
        print("")
...
def processo_1(id1, digito):
    ...
    acrecenta_cordenadas = id1 * incremento_cord  # Use a variável aqui
    while id1 < 6:
        id1 += 1
        print(f'Processo 1.{id1} ', end='', flush=True)
        selecaoprocessos()
        ...
        print(f'- Finalizado {id1} de 6', end='', flush=True)
        verificarerror()
...
def processo_2(id2, digito):
    ...
    acrecenta_cordenadas = id2 * incremento_cord  # Use a variável aqui
    while id2 < 15:
        id2 += 1
        print(f'Processo 2.{id2} ', end='', flush=True)
        selecaoprocessos()
        ...
        concluir_cadastro()
        pyautogui.click(1020, 611, duration=0.15)
        print(f'- Finalizado {id2} de 15', end='', flush=True)
        verificarerror()
        sleep_cancelamento(timeset+3)
...
def main():
    ...
    id2 = int(input("Digite o ID inicial para o Processo 2 (1 a 15): "))
    print("=" * 50 + "\n")
    processo_1(id1, digito)     # Recebimentos_materiais
    processo_2(id2, digito)     # Aquisição_Servico
